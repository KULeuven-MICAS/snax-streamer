 ...
 * 
 * The temporal address generation unit generates a new address
 * every clock cycle. The address generation is finished when
 * all the loop counters reach their loop bounds.
 * 
 * The number of nested for loops is parametrizeable at design time.
 * The loop bounds and strides are programmable at run time.
 */

/**
 * This class represents the input/output interface for the Temporal Address Generation Unit.
 *
 * @param loopDim         The dimension of the temporal loops = the number of for loops.
 * @param loopBoundWidth  The bit width of the loop bounds.
...
/**
 * TemporalAddrGenUnit is a module that generates addresses based on a nested for loop configuration.
 * The configuration can be programmed through the input interface.
 * After the configuration is programmed, the module will generate addresses 
 * until the end of the nested for loops.
 *
 * @param loopDim         The number of nested loops.
 * @param loopBoundWidth  The bit width of the loop bounds.
 * @param addrWidth       The bit width of the addresses.
 */
class TemporalAddrGenUnit(
  ...
  * configuration signals are stored and the FSM transitions to the BUSY state.
  *
  * In the BUSY state, the address generation is started. The FSM remains
  * in the BUSY state until the address generation is finished. When the
  * address generation is finished, the FSM transitions back to the IDLE state.
  */
   ...
   *
   * @param valid      The flag indicating wether to increment on this clock cycle.
   * @param loopDim    The number of neted for loops.
   * @param loopBounds The loop bounds for each for loop.
   * @return           The generated nested loop counters.
   */
